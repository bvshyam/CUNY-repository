...
Men hights are an perfect unimodel symmetric normal distribution. The mean is centered around 178.

Female hights are an unimodel symmetric normal distribution. The mean is centered around 168.

Below are the qqplot of height and weight.

```{r load-library,eval=TRUE}
qqplot(mdims$hgt,mdims$wgt)
qqplot(fdims$hgt,fdims$wgt)
```
...
**Answer Yes. The data are nearly falling into an noraml distribution**
...
Below is the histogram plot of `sim_norm`. Most of the point bins fall under normal distribution curve. But this might vary depends on the `rnorm`.

```{r sim_normprob,eval=TRUE}
hist(sim_norm,probability = TRUE)
simx <-  140:190
simy <- dnorm(simx,mean = fhgtmean,sd=fhgtsd)
lines(simx,simy,col="red")

```

Below is the QQ plot of `sim_norm`. Most of the points fall under a stright line. But this might vary depends on the `rnorm`.

```{r sim_qqplot,eval=TRUE}
...
`sim_norm` plot is more symmentrical and normally distributed compared to the real data.
...
**Yes. `fdims$hgt` look similar to the plots created for simulated data. The plots provide that the female heights are nearly normal**

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.

```{r simweights,eval=TRUE}
...
** This distribution is skewed to its right. And there were some outliers, `fdims$wgt` look similar to the plots created for simulated data. The plots provide that the female weights are nearly normal**
...
**Probability of an random female picked is above height 165?**
```{r questions,eval=TRUE}

1-pnorm(165, mean=mean(fdims$hgt),sd=sd(fdims$hgt))
```


**Probability of an random female picked is below weight 20?**

```{r questions1,eval=TRUE}

pnorm(20, mean=mean(fdims$wgt),sd=sd(fdims$wgt))

```

It seems the probability of picking a women under 20 is almost 0.
...
**C and D has `age` and `height`. These were mentioned in rounded number. It is not continuous. So it is shown in steps.**
...
** Yes. Normal probability plots can be used both to access normality and to visalize skewness. 
```{r normalprobplot, eval=TRUE}
