...
while(length(new_results) > 0){
    ...
    results <- GET("https://www.gov.uk/government/announcements?keywords=&announcement_type_option=press-releases&topics[]=all&departments[]=all&world_locations[]=all&from_date=&to_date=01%2F07%2F2010/")
...
release_corpus <- Corpus(VectorSource(release))
  ...
  docs <- c("this is shyam","this is sowmya",release,"non ego, Phoebe, datas a te mihi mentiar artes,
...
docs_corpus <- Corpus(VectorSource(docs))

VectorSource(docs_corpus[[4]])

meta(docs_corpus[[2]],"id")
meta(docs_corpus[[2]], "organisation") <- organisation
...
test <- tm_map(docs_corpus, content_transformer(tolower))

test <- tm_map(test, removeWords, stopwords("english"))

test1 <- tm_map(test, stemDocument)


meta(test[[1]], "heading") <- 'shyam'
meta(test, "heading") <- 'sowmya2'

DublinCore(test[[1]], "Creator") <- "Ano Nymous"

meta(test[[1]], "head") <- "shyam"
...
TermDocumentMatrix(test)

findFreqTerms(dtm,6)

findAssocs(dtm, "a", 0.8)



meta(test, "foo") <- letters[1:4]
